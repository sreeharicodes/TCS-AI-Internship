...
def percentile_heatmap(indices, data):
	...
	sns.heatmap(pcts, yticklabels=['Index ' + str(x) for x in indices],
	            annot=True, linewidth=.1, vmax=99, fmt='.1f', cmap='YlGnBu')
...
def pca_results(good_data, pca):
	...
	components = pd.DataFrame(
		np.round(pca.components_, 4), columns=good_data.keys())
